...
export function WebviewModal({ url, source, path, onClose }: { url?: string; source?: number | string; path?: string; onClose: () => void }) {
  ...
  // Pointer drag = wheel scroll (mouse-only, like JCEF — no touch, so no double-tap zoom). A drag that
  // barely moved is treated as a tap → click. Horizontal (dx) still works on overflow-scrollable rows.
